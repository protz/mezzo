...
(* A renaming is a mapping of internal names to external names. This sort
   of renaming is used when opening an abstraction. *)
...
module Make (E : EXPRESSION) = struct
  ...
  (* The internal version of [bind i x e] replaces the external name [x]
     with the internal index [i] throughout the expression [e]. *)
  ...
  (* [push delta rho e] requires [delta/rho/e] to be 0-closed. Its effect
     is to apply [delta/rho] to [e], all the way to the leaves, producing
     a standard expression. *)

  let rec push (delta : int) (rho : 'x renaming) ((E e) : 'x expr) : 'x expr =
    E (
      E.map
