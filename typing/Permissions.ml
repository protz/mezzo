...
let safety_check env =
  ...
  if Log.debug_level () >= 5 then begin
    fold_terms env (fun () var permissions ->
      ...
      let exclusive = List.filter (FactInferenceBis.is_exclusive env) permissions in
...
and keep_only_duplicable env =
  let env = fold_terms env (fun env var permissions ->
    let permissions = List.filter (FactInferenceBis.is_duplicable env) permissions in
  ...
  let floating = List.filter (FactInferenceBis.is_duplicable env) floating in
...
and add (env: env) (var: var) (t: typ): env =
  ...
  | TyConcreteUnfolded (dc, ts, clause) ->
      ...
      | Some (ts', clause') ->
          ...
          with
          | _ when FactInferenceBis.is_exclusive env t ->
...
and add_type (env: env) (p: var) (t: typ): env =
  ...
  | Some _ ->
      ...
      in
      if FactInferenceBis.is_exclusive env t then begin
        ...
        Log.debug ~level:4 "%sInconsistency detected%s, adding %a as an exclusive \
          ...
          TypePrinter.ptype (env, t);
        mark_inconsistent env
      end else if FactInferenceBis.is_duplicable env t && in_there_already then
        env
      else
  ...
  | None ->
      ...
      if FactInferenceBis.is_exclusive env t then
...
and sub (env: env) (var: var) (t: typ): env option =
  ...
  if is_inconsistent env then
    ...
    let sort = function
      | _ as t when not (FactInferenceBis.is_duplicable env t) -> 0
    ...
    | Some (remaining, (t_x, env)) ->
        ...
        if FactInferenceBis.is_duplicable env t_x then
          Some env
        else
...
and sub_constraints env constraints =
  List.fold_left (fun env (f, t) ->
     ...
     *)
    let f' = FactInferenceBis.analyze_type env t in
...
and sub_floating_perm env t =
  ...
  | Some (remaining_perms, (t', env)) ->
      if FactInferenceBis.is_duplicable env t' then
        Some env
      else
