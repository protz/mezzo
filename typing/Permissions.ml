...
(*type tristate = {
  (* All the points that are in the parent environment. *)
  parent_points: point list;
  env_a: env;
  env_b: env;
  env_c: env
}

let merge (parent_env: env) (child_envs: env list): env =
  let points = fold_terms parent_env (fun acc point _ -> point :: acc) [] in
  let empty_env = fold_terms
    parent_env
    (fun empty_env point _ -> replace_term empty_env (fun raw -> { raw with permissions = [] }))
    parent_env
  in
  List.fold_left (merge_envs points empty_env) (List.hd child_envs) (List.tl child_envs)


and merge_envs (parent_points: point list) (empty_env: env) (env_a: env) (env_b: env): env =
  let tristate = { env_a; env_b; env_c = empty_env; parent_points } in
  let tristate = List.fold_left merge_point tristate work_list in
  tristate.env_c


and merge_point: (tristate: tristate) (point: point): tristate =
  let { env_a; env_b; env_c } = tristate in
  let types_a = get_permissions env_a point in
  let types_b = get_permissions env_b point in
  List.fold_left (fun tristate type_a ->
    List.fold_left (fun tristate type_b ->
      match merge_types tristate type_a type_b with
      | None ->
          tristate
      | Some (tristate, t) ->
          let env_c = Permissions.add tristate.env_c point t in
          { tristate with env_c }
    ) tristate types_b
  ) tristate types_a


(* This function should be symmetrical with respect to its [t_a] and [t_b]
 * arguments. *)
and merge_types (tristate: tristate) (t_a: typ) (t_b: typ): (tristate * typ) option =
  assert (false);*)
