...
let rec check_expression (env: env) ?(hint: name option) ?(annot: typ option) (expr: expression): env * var =
      ...
      | Some (remaining_perms, (perm, clause)) ->
          if equal env clause ty_bottom then
            ...
            | Some t ->
                 ...
                 * legit. *)
                if not (FactInferenceBis.is_exclusive env t) then
                  raise_error env (BadFactForAdoptedType (y, t, FactInferenceBis.analyze_type env t));
            ...
            Log.check (FactInferenceBis.is_exclusive env clause)
  ...
  | EOwns (y, x) ->
	 ...
	 false. *)
      if not (List.exists (FactInferenceBis.is_exclusive env) (get_permissions env y)) then
        raise_error env (NoAdoptsClause y);
      return env !*t_bool
