  ...
  let make_base_envs ?annot () =
    ...
    let mark_duplicable_vars env =
      fold_terms env (fun env var permissions ->
        if List.exists (fun x -> not (FactInferenceBis.is_duplicable env x)) permissions then
          mark env var
        else
          env
      ) env
  ...
  and merge_lists
    ...
    | left_perm :: left_perms, remaining_right_perms ->
        ...
        | Some (remaining_right_perms, (right_perm, (left_env, right_env, dest_env, dest_perm))) ->
            ...
            let left_is_duplicable = FactInferenceBis.is_duplicable left_env left_perm in
            let right_is_duplicable = FactInferenceBis.is_duplicable right_env right_perm in
